...
Vitum Lab (`vitumlab.com`) is a research peptide e-commerce site selling GLP-3 (R) / Retatrutide, GHK-Cu, NAD+, and BAC Water. Checkout is dual-processor: **PayRam** (primary — card/Apple Pay/crypto, self-hosted on a dedicated VPS, non-custodial, settles USDC/USDT) + **NowPayments** (crypto-only fallback). Deployed on Vercel.

**Payment architecture decision (June 2026):** PayRam is the planned primary card processor. It is self-hosted (Docker on Ubuntu VPS — cannot run on Vercel or serverless), non-custodial, requires no KYB, and accepts cards + Apple Pay + crypto (settles in USDC/USDT). NowPayments remains as the crypto-only fallback path. **Status: PayRam integration not yet implemented** — awaiting VPS provisioning and PayRam API URL + key from the operator. When ready, fold the PayRam checkout path into `api/admin/[...slug].ts` or an existing catch-all (do NOT add a 13th Vercel function — already at the 12-function limit).
