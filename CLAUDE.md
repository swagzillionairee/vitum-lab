...
**Security — outstanding owner actions (dashboards, not code):** (1) Supabase Auth → confirm "Confirm email" ON + enable leaked-password protection; (2) Google Cloud → restrict `VITE_GOOGLE_MAPS_API_KEY` by HTTP referrer; (3) move `pg_net` out of the `public` schema. *Deferred code follow-ups:* CSP header (needs per-source enumeration for Maps/Supabase/Square); stop logging raw customer emails.

**Security audit (July 2026) — fixed in `20260714000000` + code:** (1) `decrement_promo_use` / `stamp_email` / `expire_stale_orders` were `SECURITY DEFINER` but left EXECUTE-able by anon/authenticated (the hardening batch omitted the REVOKE) — anyone with the anon key could defeat a promo cap, suppress an order's emails, or force the expiry sweep; now REVOKEd to service_role only (applied live). (2) Rate limiter keyed on the spoofable **leftmost** `x-forwarded-for` hop → new `_lib/clientIp.ts` prefers `x-real-ip` / last hop; waitlist gains a per-email bucket. (3) `create-crypto-payment` + `validate-discount` now rate-limited per JWT user; the Shippo address check runs *after* the cheap cart/stock checks. (4) Duplicate `cartCode` lines are aggregated before the stock check (oversell fix). (5) `requireAffiliate` + `me.ts` now filter `is_referral=false` so self-serve referral participants can't reach the curated-affiliate endpoints. (6) `/api/me` welcome-email dedupe is atomic via the new `welcome_sent` claim table. (7) `cron` reconciles confirmed Square card orders against refunds/chargebacks (`getSquareReversal` → `refundClawback`) since there's no Square webhook; `refundClawback` flags a residual store-credit deficit. (8) Login `?redirect=` rejects protocol-relative (`//`, `/\`) targets.
