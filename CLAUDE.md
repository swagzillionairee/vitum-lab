...
- `discount_redemptions(email, code, order_id, created_at, PK(email,code,order_id))` — atomic per-customer backstop for promo + referral: `reserve_discount_redemption(email, code, order_id, limit)` counts slots per (email,code) under an advisory lock (limit from `per_customer_limit`; referral uses sentinel `__REFERRAL__` = one referral ever per referee). Claimed at checkout, released on abandonment + hourly sweep, cleared when its promo is deleted.
- `rate_limits(id BIGINT PK, bucket, created_at)` — sliding-window limiter (contact form).

**Key RPCs:** `confirm_order_paid` / `cancel_order` (the WHOLE payment claim + stock + promo mutation as one Postgres transaction — cancel restocks only unfulfilled paid orders); `decrement_stock`/`increment_stock` (atomic, raise on insufficient); `claim_email`/`release_email_claim` (atomic claim-before-send email idempotency); `record_affiliate_payout` (payout capped at outstanding under advisory lock); `store_credit_balance` (excludes dead-order entries); `reserve_store_credit` (atomic check-and-reserve under advisory lock; idempotent per order); `reserve_discount_redemption`/`release_discount_redemption`/`sweep_discount_redemptions`; `rate_limit_hit`.

**`payment_config` extras:** `blocked_states` (array of 2-letter codes) — ship-to-state blocklist enforced server-side at checkout (no admin UI yet; set via SQL or ask Claude). Manual-method `handle`/`instructions` are stripped from `/api/public/site` while a method is disabled.
...
- **`pnpm check` now type-checks `api/` too** (`tsconfig.api.json`), and CI (`.github/workflows/ci.yml`) gates PRs on `pnpm check` + `pnpm test`. The ESM rule still applies: relative imports in `api/` **must** keep `.js` extensions or Vercel fails at runtime.
...
- **Shipping (Shippo, USPS)** — `buy_label` → USPS Ground Advantage (weight/zone priced; parcel = `GA_PARCEL` in `_lib/shippo.ts`, tune weight to real avg), 4×6 PDF; stores label/tracking, sets shipped, emails. **Auto-delivery:** `/api/cron` polls tracking → marks delivered + emails. Token decides test/live (currently TEST → sample labels).
...
- **SEO** — keywords/OG/Twitter/JSON-LD (Product with `shippingDetails`+`priceValidUntil` per offer + BreadcrumbList on product pages, FAQPage on /faq, Organization/WebSite on home); static OG/Twitter fallbacks in `index.html` (crawlers don't run JS); complete `sitemap.xml` (all 9 products + dose calculator) + `robots.txt`. Merchant return-policy schema deliberately omitted pending owner's return-policy terms.
- **July 2026 audit batch** — cart persists in **localStorage** (magic-link round-trip no longer wipes it; sessionStorage read once as migration); BAC Water one-tap **cross-sell** in the cart drawer (requires explicitly positive stock); **dose calculator is public** (nav + PDP + canonical — organic-search entry point); OOS grid cards link to the PDP waitlist form (`#notify`); checkout **two-stage $75/$100 nudge**, payment errors render under the pay button, crypto redirect guards a missing invoiceUrl, `/api/public/site` load failure auto-retries + offers Try again; manual-modal copy reframed (no "automatic refund" threat); **a11y**: skip link, dialog roles, keyboard marquee pause, labelled inputs, aria-pressed tiles, red-600 error contrast; shared `fetchSiteConfig()` dedupes the per-page site-config fetch; **account data-export** (`GET /api/account/data-export`) + admin order **redact** action (PII strip, financial row preserved); `notifyAdmin()` ops alerts (Square payment anomalies); `requireEnv()` for actionable missing-env errors; client/server **pricing-parity test** locks the mirrored math together.
...
**Testing:** Vitest unit/component (incl. client/server pricing-parity) + a Playwright checkout e2e. CI (`.github/workflows/ci.yml`) runs `pnpm check` + `pnpm test` on every PR and push to main. The e2e suite is not in CI (needs browsers; occasionally flaky under constrained containers — passes in isolation).

---

## Owner Risk Register (July 2026 audit — decisions/actions only the owner can take; do NOT build around them without sign-off)
...
2. **Data disaster recovery:** orders + store-credit + affiliate-payout ledgers live in ONE Supabase project behind one service key, with no backup story. Action: enable PITR (paid tier) and/or a nightly logical export of `orders` + ledger tables (can fold into `/api/cron` once a destination exists); write a restore runbook.
3. **Email deliverability:** all transactional mail rides one Gmail app-password (no SPF/DKIM/DMARC story; ~1.5-2k/day cap; the waitlist blast is burst-shaped). A confirmation in spam reads as "my charge failed" → chargebacks. Action: move to an ESP (Resend/Postmark/SES) on a sending subdomain; keep `sendEmail()` as the seam. The waitlist blast should be batched when this lands.
4. **Analytics:** none exist — conversion rate, funnel drop-off, and affiliate/referral ROI are unmeasurable. Owner picks a privacy-first vendor (Plausible/PostHog recommended; a Google/Meta pixel risks ad-account bans in this category) and supplies the key; wiring is ~1 day and gates on the existing cookie-consent value (which nothing reads today).
...
6. **PII retention:** admin `redact` + customer data-export exist, but no automatic retention window purges old shipping addresses. Decide a window (e.g. anonymize N months post-delivery) before automating.
7. **Fraud/chargeback controls:** checkout has a rate limiter but no order-value ceiling, per-card velocity, or AVS/CVV rejection policy. Decide thresholds before the card volume grows.
8. **Function ceiling:** 12/12 Vercel Hobby functions — zero headroom; new endpoints must fold into catch-alls. Cheapest unlock is Vercel Pro; alternative is consolidating through the Express server as one function.
9. **Sales tax:** unchanged — no tax line at checkout; owner to confirm obligation with a tax pro (single pricing change in `create-crypto-payment` if required).
