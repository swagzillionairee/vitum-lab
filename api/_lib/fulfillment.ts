 ...
 * used by the PayRam webhook (api/public/payram-webhook). The NowPayments
 * webhook still has an equivalent inline copy; both can converge here once
 * PayRam is validated in production. Every step is idempotent so duplicate
 * webhooks are safe.
 */
